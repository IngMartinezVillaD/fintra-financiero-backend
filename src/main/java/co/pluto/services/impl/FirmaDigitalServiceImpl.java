...
import co.pluto.config.ThomasSigneProperties;
...
public class FirmaDigitalServiceImpl implements IFirmaDigitalService {
  ...
  // ─────────────────────────────────── firma manual (sin integración)

  @Override
  @Transactional
  public FirmaEstadoDto marcarFirmadaManual(Long operacionId) {
    OperacionEntity op = findOperacionOrThrow(operacionId);
    if (!"FD".equals(op.getEstadoPipeline()))
      throw new BusinessException("Solo se puede marcar como firmada una operación en estado FD");

    ThomasSigneSolicitudEntity solicitud = solicitudRepo
        .findByIdempotencyKey("FIRMA-" + operacionId)
        .orElseGet(() -> {
          ThomasSigneSolicitudEntity s = new ThomasSigneSolicitudEntity();
          s.setOperacionId(operacionId);
          s.setIdempotencyKey("FIRMA-" + operacionId);
          s.setDestinatarioEmail("manual@pluto.co");
          s.setDocumentoUrl("manual");
          return s;
        });
    ...
    solicitud.setFirmadoAt(OffsetDateTime.now());
    op.setFirmaDigitalAt(OffsetDateTime.now());
    registrarEvento(op, "FD", "FD", "Firma marcada manualmente sin integración Thomas Signe — pendiente desembolso.");

    solicitudRepo.save(solicitud);
    operacionRepo.save(op);
    log.info("Operación {} marcada como firmada manualmente.", op.getId());
    return toDto(solicitud);
  }
  ...
  private byte[] generarPdf(OperacionEntity op) throws Exception {
    try (var baos = new ByteArrayOutputStream()) {
      ...
      doc.add(new Paragraph("Generado automáticamente por Pluto — " + LocalDate.now(), fontSmall));
  ...
  private co.pluto.models.entity.UsuarioEntity currentUser() {
