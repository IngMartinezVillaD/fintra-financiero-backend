...
import co.pluto.dto.response.liquidacion.LiquidacionDetalleItemDto;
...
import co.pluto.services.impl.liquidacion.*;
import co.pluto.services.interfaces.ILiquidacionService;
...
public class LiquidacionServiceImpl implements ILiquidacionService {
  ...
  public LiquidacionMensualResponseDto marcarContabilizada(Long id) {
    ...
    if (!"APROBADA".equals(liq.getEstado()))
      throw new BusinessException("Solo se puede contabilizar una liquidación APROBADA");

    contabilizacionService.contabilizarLiquidacion(liq);

    liq.setEstado("CONTABILIZADA");
    liq = liqRepo.save(liq);
    log.info("Liquidación {}/{} contabilizada — asientos generados", liq.getMes(), liq.getAnio());
    return toDto(liq, detalleRepo.findAllByLiquidacionId(id));
  }
